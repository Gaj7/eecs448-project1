 ...
 // 	std::string m_str(m_char);
  ...
  std::string y_str = std::to_string(m_curdate[0]);
  std::string m_str = std::to_string(m_curdate[1]);
  std::string d_str = std::to_string(m_curdate[2]);
 ...
 // 	std::string d_str(d_char);
 	...
 	for(int i=0; i<dll->size(); i++){ //traverse dll
 		...
 		if(tempvec.size() != 0){ //check if there are details
          ...
          std::string y_str = std::to_string(y_int);
          std::string m_str = std::to_string(m_int);
          std::string d_str = std::to_string(d_int);
